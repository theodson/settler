#!/usr/bin/env bash
#
# provision homestead 11.6.0 (php-8.3 introduced and hopefully postgresql-18).
#
if [ $# -gt 0 ] && [ $1 = "config" ]; then
    CONFIG_ONLY=1
fi

MAINTAIN_PHP_AT_VERSION="${MAINTAIN_PHP_AT_VERSION:-8.3}"
MAINTAIN_NODE_AT_VERSION="${MAINTAIN_NODE_AT_VERSION:-17}"
MAINTAIN_POSTGRESQL_AT_VERSION="${MAINTAIN_POSTGRESQL_AT_VERSION:-9.5}"
...
rpm_versions pre_provision_11.6.0
#disable_blackfire
